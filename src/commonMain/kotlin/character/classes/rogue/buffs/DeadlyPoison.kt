...
import character.*
import character.classes.rogue.abilities.DeadlyPoison
...
class DeadlyPoison(sourceItem: Item) : Poison(sourceItem) {

    override val name = "Deadly Poison ${sourceItem.uniqueName}"
    ...
    val proc = object : ItemProc(sourceItems) {
        ...
        override fun proc(sp: SimParticipant, items: List<Item>?, ability: Ability?, event: Event?) {
            if(poisonAbility == null) {
                ...
                val name = "Deadly Poison $suffix"
                poisonAbility = DeadlyPoison(name, sourceItem)
            }
